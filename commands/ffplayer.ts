...
import { ClashRoyaleAPI, Player, Card } from '../types';

const functions = require('../utils/functions.js'); async function ffplayer(
    bot: Client,
    api: ClashRoyaleAPI,
    ...
    if (interaction !== null) {
        ...
        details = interaction.options.getBoolean('details') ?? false;
    }

    if (!tag) {
        functions.errorEmbed(bot, interaction, channel, "Tag is required");
        return;
    }

    const regex: RegExp = /\#[a-zA-Z0-9]{6,9}\b/g;
    ...
    const playerEmbed: any = functions.generateEmbed(bot);
    ...
    const yearsPlayed: number = player.badges[0]?.level ?? 0;
    ...
    player_data += `<:Hashtag:1186369411439923220> Tag: **${player.tag}**\n`
        ...
        + `:old_man: Years played: **${yearsPlayed}** year${yearsPlayed > 1 ? 's' : ''}\n`;
    ...
    if (interaction && details) {
        player_data += `\nExp Points: ${player.expPoints}\n`
            ...
            + `Current deck: \n`
            + player.currentDeck.map((card: Card) => ` - ${card.name}`).join('\n');
    }
...
export async function execute(
    ...
    await ffplayer(bot, api, interaction, null, null);
}
